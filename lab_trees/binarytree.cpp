...
template <typename T>
bool BinaryTree<T>::isOrdered(const Node* subRoot, const Node* lower, const Node* upper) const
{
  if (!subRoot) return true;

  if (lower && subRoot->elem < lower->elem) return false;
  if (upper && subRoot->elem > upper->elem) return false;
  if (!isOrdered(subRoot->left, lower, subRoot)) return false;
  if (!isOrdered(subRoot->right, subRoot, upper)) return false;
