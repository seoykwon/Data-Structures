...
private:
    ...
    //checks if a 7 x 7px box around start will have a direction out of bounds
    bool isInBound(string direc);

    //traversed 2D vector and returns item in given location <y,x>
    template <class T>
    T trav2DArray(vector<vector<T>> v, pair <int,int> loc);

    //draws 7x7px square around start
    void drawSquare(PNG *im);

    bool outOfBound(int x, int y);

    void setFifty(PNG &im, pair<int, int> loc, int d);


// ========= private member variables ================

	pair<int,int> start;  // col,row (y, x)
