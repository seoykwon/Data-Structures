...
//   };
   ...
   */
  int size();
  // pair<int, int> splitLocation(stats &s, pair<int, int> ul, int w, int h);
 ...
 private:
  ...
  // void copyTree(Node * current, const Node * other);
  // void clearTree(Node* & current);
  // int sizeTree(const Node * current);
  // void render(Node * current, PNG & result);
  // pair<int, int> splitLocation(stats &s, pair<int, int> ul, int w, int h);
