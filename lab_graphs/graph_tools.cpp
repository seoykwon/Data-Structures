...
int GraphTools::findMinWeight(Graph& graph)
{
    ...
    for (const Vertex& v : graph.getVertices()) graph.setVertexLabel(v, "UNEXPLORED");
    
    for (const Edge& e : graph.getEdges()) graph.setEdgeLabel(e.source, e.dest, "UNEXPLORED");
...
int GraphTools::findShortestPath(Graph& graph, Vertex start, Vertex end)
{
    ...
    while (end != start) {
        ...
        //setLabel(end, parent[end], "MINPATH"); ?
        end = parent[end];
        ans++;
    }
    return ans;
