...
RGBAPixel* myFavoriteColor(int blueval) {
    ...
    // return &pixel; why not work? stack?
    // memory leak happens since we do not exactly know the address

    RGBAPixel* pixel = new RGBAPixel(64, 191, blueval);
    return pixel; //return a copy
}
...
void sketchify(std::string inputFile, std::string outputFile) {
    ...
    for (unsigned y = 1; y < height - 1; y++) { //why not height
        for (unsigned x = 1; x < width - 1; x++) { //why not width
            ...
            if (score > 0.3) {
                *currOutPixel = *myPixel;
                //currOutPixel = myPixel;
                //changing pointers does not change pixel value directly
                //you are just changing address
            }
        }
    }
